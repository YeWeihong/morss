...
# 示例：我是使用 USB 存储，目录可以自定义，但是记得要在docker-compose文件中的缓存部分也修改
mkdir -p /mnt/usb1-1/morss/morss_custom_test/morss_cache
```

#### 第三步：编写 docker-compose.yml

使用clone下来morss目录中的docker-compose.yml，当然你可以自己修改你想要的
...
```bash
# 首次构建（时间较长，请耐心等待，我用过这个，能用）
docker-compose build

# 或者等价于（AI生成的，我没用过这个命令）
