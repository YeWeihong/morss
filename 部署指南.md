...
> **快速参考**：
> - 快速命令参考：**[快速部署.md](快速部署.md)**
> - **如何修改端口（如改为 18000）**：**[PORT_CONFIGURATION.md](PORT_CONFIGURATION.md)**
...
本仓库已包含 `docker-compose.yml` 文件和 `.env.example` 配置示例。
...
如果你想修改 Docker 占用的端口（例如改用 18000 端口而不是默认的 8000 端口），有以下几种方法：
...
# 或者在 .env 文件中设置
echo "HOST_PORT=18000" > .env
docker-compose up -d

# 访问：http://localhost:18000
```

**方法二：直接修改 docker-compose.yml**

编辑 `docker-compose.yml`，修改 ports 配置：

```yaml
ports:
  - "18000:8000"  # 主机端口:容器端口
```

**方法三：命令行直接指定**

```bash
# 不使用 docker-compose，直接运行
...
**重要说明**：
- Docker 端口映射格式为 `主机端口:容器端口`
- 容器内部始终使用 8000 端口（由 `PORT` 环境变量控制，默认 8000）
- 你只需要修改**主机端口**（冒号前面的数字）
- 例如 `18000:8000` 表示：主机的 18000 端口映射到容器的 8000 端口
- 修改后通过 `http://localhost:18000` 访问服务
