...
本节以斐讯N1（ARM64 架构）为例，介绍如何在家用低功耗设备上通过 Docker Compose 从源码构建并运行 Morss。其他运行 Linux + Docker 的家用设备（如树莓派、Odroid 等）步骤类似。

> **前提条件**：斐讯N1 已刷入支持 Docker 的 Linux 系统（如 Armbian），且已安装 `docker` 和 `docker-compose`。

#### 第一步：克隆仓库

```bash
# SSH 到你的 N1 盒子，然后克隆本仓库
git clone https://github.com/YeWeihong/morss.git
cd morss
```
...
```bash
# 建议使用外部 USB 存储或 emmc 上的固定路径
# 示例：使用 USB 存储
mkdir -p /mnt/usb1-1/morss_custom/morss_cache
```

#### 第三步：编写 docker-compose.yml

在仓库根目录创建（或替换）`docker-compose.yml`：
...
services:
  morss:
    ...
    # 使用本地构建的自定义镜像
    image: morss:custom
    container_name: morss_custom
    restart: unless-stopped

    # 使用默认 Bridge 网络模式并映射端口
    # 格式：宿主机端口:容器内部端口
    ports:
      - "18000:18000"

    environment:
      # 如果你的网络环境需要代理，填入代理地址；否则可删除这两行
      - HTTP_PROXY=http://192.168.5.2:7890
      - HTTPS_PROXY=http://192.168.5.2:7890

      # 让容器内部监听 18000 端口（需与 ports 映射保持一致）
      - PORT=18000
      ...
      # Gunicorn 性能调优
      # N1 为双核/四核设备，workers=2 是合理的初始值
      - GUNICORN_CMD_ARGS=--workers=2 --timeout=120

    volumes:
      # 持久化缓存目录（宿主机路径:容器内路径）
      - /mnt/usb1-1/morss_custom/morss_cache:/cache

      # （可选）挂载自定义 www 目录，覆盖内置静态资源
      # - /mnt/usb1-1/morss_custom/morss/www:/app/custom_www

    healthcheck:
      test: ["CMD", "/bin/sh", "/app/morss-helper", "check"]
      interval: 5m   # 每 5 分钟检查一次，避免日志刷屏
      timeout: 10s
      retries: 3
```
...
| 配置项 | 说明 |
|--------|------|
| `build: .` | 从当前目录的 Dockerfile 构建镜像，适合 ARM 等非 amd64 架构（官方镜像可能不支持） |
| `image: morss:custom` | 为本地构建的镜像命名，方便后续管理 |
| `network_mode: host`（已移除） | N1 上使用 Bridge 模式更稳定；如使用 `host` 模式，需删除 `ports` 配置 |
| `ports: "18000:18000"` | Bridge 模式必须配置端口映射，格式为 `宿主机端口:容器内端口` |
| `PORT=18000` | 控制 Gunicorn 在容器内监听的端口，需与 `ports` 的容器端口一致 |
| `HTTP_PROXY` / `HTTPS_PROXY` | 若 N1 本身需要通过代理访问外网，填入代理地址；否则删除 |
| `CACHE_SIZE=1073741824` | 缓存最大 1GB，可根据存储空间调整 |
| `GUNICORN_CMD_ARGS=--workers=2` | N1 资源有限，建议 workers 设置为 2；如有更多核心可适当提高 |
| `volumes` | 将缓存目录挂载到宿主机，重启容器后缓存不丢失 |
...
> **ARM 构建提示**：Dockerfile 使用 `alpine:edge`，支持 ARM64/ARM32 架构，在 N1 上可直接构建，无需额外配置。
...
```bash
# 在 N1 本机测试（替换 18000 为你配置的端口）
curl http://localhost:18000/

# 在局域网内其他设备访问（替换 N1 的 IP）
# 浏览器打开：http://192.168.x.x:18000/
```

服务正常启动后，浏览器应能看到 Morss 的 Web 界面。
...
# 进入容器调试
docker exec -it morss_custom /bin/sh

# 查看健康检查状态
docker inspect --format='{{.State.Health.Status}}' morss_custom
```
...
# 查看容器日志找到错误信息
docker logs morss_custom
```

**代理不生效**：
- 确认 `HTTP_PROXY` / `HTTPS_PROXY` 中填写的是宿主机可访问的代理地址
- N1 的 Docker Bridge 网络中，宿主机通常为 `172.17.0.1`，局域网代理地址参考实际配置

**缓存目录权限问题**：
```bash
# 容器内 UID 为 1000，确保宿主机缓存目录有写入权限
sudo chown -R 1000:1000 /mnt/usb1-1/morss_custom/morss_cache
```
